...
#include "ata_volt_outer_thread.h"
...
static void finalize_block(struct block_info *bi)
{
  ...
  char dropstat[128];
  //TODO Aug 5 - this check is invalid, since we use different packet numeration
  // We actually have NANTS * FREQCHAN packets per block
...
int ata_obs_info_init(struct ata_obs_info * info)
{
    ...
    {
        info->antids[iK] = ATA_ANT_NOT_INITED;
    }
    return 0;
}

//get antenna index from the obs info
int get_ata_ant_index(const struct ata_obs_info info, const int32_t antid)
{
    int retv = -1, iK;
    if(info.nants >= MAXANTS)
    {
        return retv;
    }
    for (iK=0; iK<info.nants;++iK)
    {
        if(info.antids[iK] == antid)
        {
            retv = iK;
            break;
        }
...
static int init(hashpipe_thread_args_t *args)
{
  ...
  if(hpguppi_pktbuf_slot_offset(dbin, PKT_OFFSET_ATA_VOLT_HEADER) %
      PKT_ALIGNMENT_SIZE != 0
  || hpguppi_pktbuf_slot_offset(dbin, PKT_OFFSET_ATA_VOLT_PAYLOAD) %
      PKT_ALIGNMENT_SIZE != 0) {
    errno = EINVAL;
    hashpipe_error(thread_name, "IBVPKTSZ!=%d,%d,[...]",
        PKT_OFFSET_ATA_VOLT_HEADER, PKT_OFFSET_ATA_VOLT_PAYLOAD -
        PKT_OFFSET_ATA_VOLT_HEADER);
  ...
  hashpipe_status_lock_safe(st);
  {
    ...
    // If CHAN_BW is zero, set to default value
    if(chan_bw == 0.0) {
      chan_bw = 900.0/4096.0;
    }
    ...
    // Force PKTFMT to be "ATAVOLT"
    hputs(st->buf, "PKTFMT", "ATAVOLT");
...
int debug_i=0, debug_j=0;
  ...
  // blocks)
  struct block_info wblk[VOLT_OUTER_HASHPIPE_BLOCKS_AT_A_TIME];
  int wblk_idx;
  ...
  uint8_t * p_u8pkt;
  struct ata_ibv_volt_pkt * p_spdpkt = NULL;

  // Structure to hold observation info, init all fields to invalid values
  struct ata_obs_info obs_info;
  ata_obs_info_init(&obs_info);
  ...
  for(wblk_idx=0; wblk_idx< VOLT_OUTER_HASHPIPE_BLOCKS_AT_A_TIME ; wblk_idx++) {
  ...
  hashpipe_status_lock_safe(st);
  {
    ...
    hgetu4(st->buf, "FREQCHAN", &obs_info.nchan);
    ...
    if(ata_obs_info_valid(obs_info)) {
      // Update obsnchan, pktidx_per_block, and eff_block_size
      pktidx_per_block = ata_pktidx_per_block(BLOCK_DATA_SIZE, obs_info);
      eff_block_size = ata_block_size(BLOCK_DATA_SIZE, obs_info);
    ...
    hputu4(st->buf, "FREQCHAN", obs_info.fenchan);
  ...
  while (run_threads()) {
    ...
    do {
      ...
      if(curtime != lasttime) {
        ...
        //TODO Aug5: - we are receiving from multiple snaps, whole section need changes, haven't figured out best solution
...
#else
      p_spdpkt = (struct ata_ibv_volt_pkt *)p_u8pkt;
#endif
...
static hashpipe_thread_desc_t ata_thread_desc = {
    name: "ata_volt_outer_thread",
    skey: "NETSTAT",
    init: init,
    run:  run,
...
{
  register_hashpipe_thread(&ata_thread_desc);
}
